...
export const articles: Article[] = [
	...
	{
		slug: 'add-passkeys-to-nodejs-web-app',
		title: 'How to Add Passkeys to a Node.js Web App Without Locking Users Out',
		seoTitle: 'Add Passkeys to a Node.js Web App | PilotLab',
		dek: 'A practical WebAuthn rollout for product teams: model credentials, verify challenges on the server, preserve recovery, and test the browser and origin boundaries.',
		published: '2026-09-11',
		updated: '2026-09-11',
		readTime: '11 min read',
		category: 'Application security',
		keyword: 'how to add passkeys to a Node.js web app',
		intro: "Passkeys can remove password-reset friction and make phishing-resistant sign-in available inside a normal web application. The browser API is only the visible part of the feature, though. A reliable implementation needs a server-generated challenge, a credential model, origin and relying-party checks, a recovery path, and tests for the cases where a user changes devices or loses a credential.",
		relatedService: { label: 'API and platform engineering', href: '/services/api-platform-engineering' },
		sources: [
			{ label: 'MDN — Passkeys', url: 'https://developer.mozilla.org/en-US/docs/Web/Security/Authentication/Passkeys' },
			{ label: 'MDN — Web Authentication API', url: 'https://developer.mozilla.org/en-US/docs/Web/API/Web_Authentication_API' },
			{ label: 'FIDO Alliance — Passkeys', url: 'https://fidoalliance.org/passkeys/' },
		],
		sections: [
			{
				heading: 'Understand the boundary before installing a library',
				paragraphs: [
				...
				bullets: ['An HTTPS origin in production; localhost is useful for local development', 'A stable relying-party ID that matches the domain scope you intend', 'A database table for users, credentials, and short-lived challenges', 'A session or token system that already has clear authorization rules', 'A maintained server library and browser support policy'],
			},
			{
				heading: 'Design the credential and challenge records',
				paragraphs: [
				...
				bullets: ['Unique constraint on credential ID', 'Explicit tenant or account ownership on every credential lookup', 'Single-use challenge state with an expiry and flow type', 'A record of created, renamed, revoked, and last-used timestamps', 'No private keys, raw assertion secrets, or unnecessary personal data in your database'],
			},
			{
				heading: 'Implement registration as a two-step server flow',
				paragraphs: [
				...
				bullets: ['Do not let the client choose the relying-party ID or expected origin', 'Reject a reused, missing, expired, or flow-mismatched challenge', 'Use an immutable account identifier for the WebAuthn user handle', 'Require recent authentication before adding a credential to an existing account', 'Handle duplicate-registration errors without exposing another account'],
			},
			{
				heading: 'Implement sign-in and session creation separately',
				paragraphs: [
				...
				bullets: ['Use a one-time authentication challenge and consume it on success or expiry', 'Compare the expected origin and relying-party ID on the server', 'Decide whether user verification is required for your risk level', 'Rotate or establish the session only after verification completes', 'Keep password or another recovery method during a measured migration'],
			},
			{
				heading: 'Plan recovery, migration, and device changes',
				paragraphs: [
				...
				bullets: ['Enroll at least one backup credential where the account risk warrants it', 'Show the last-used time and a user-editable label', 'Require reauthentication before credential management changes', 'Rate-limit recovery and record support actions with an audit trail', 'Test new-device sign-in, credential revocation, and account recovery'],
			},
			{
				heading: 'Verify the rollout with adversarial tests',
				paragraphs: [
				...
				bullets: ['A challenge cannot be replayed or used in the wrong flow', 'An assertion from another origin or account is rejected', 'Duplicate credential registration has no partial write', 'Revoked credentials cannot create a new session', 'Multiple concurrent verification attempts have one well-defined result', 'Recovery works without silently weakening high-risk accounts', 'Logs contain correlation IDs and reason codes, not raw credential material'],
			},
		],
	},
];
